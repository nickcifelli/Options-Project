...
def _cache_path(ticker: str, cache_dir: Path) -> Path:
    ...
    return cache_dir / f"{ticker.upper()}_{today}.csv"
...
def fetch_chain(
    ...
    volume, openInterest, and spot. One cached snapshot per (ticker, day).
    """
    ...
    if use_cache and cache_file.exists():
        return pd.read_csv(cache_file, parse_dates=["expiry"])
    ...
    cleaned = clean_chain(raw, min_volume, min_open_interest)
    cleaned.to_csv(cache_file, index=False)
    return cleaned
